...
#include <afxext.h>
#include "..\STLViewer\STLViewerDoc.h"
#include "..\STLViewer\STLViewerView.h"
...
protected:
	...
	DECLARE_DOCUMENT(CSTLViewerDoc)
...
public:
	virtual HRESULT InitializeDocumentPreview(HWND hWndParent, RECT* prc)
	{
		...
		CCreateContext ctx;
		ctx.m_pNewViewClass = RUNTIME_CLASS(CSTLViewerView);
		...
		m_pDocument->AddRef();
		ctx.m_pCurrentDoc = DYNAMIC_DOWNCAST(CSTLViewerDoc, (CObject*) m_pDocument->GetContainer());
