...
public:
    ...
    std::vector< std::vector<CvPoint2D64f> >dataset;	//点集合
    std::vector<LineType> lineTypeSet; //线的类型
    ...
    void xlabel(std::string xlabel_name, CvScalar label_color);
    void ylabel(std::string ylabel_name, CvScalar label_color);
    //清空图片上的数据
    void clear();
    void title(std::string title_name,CvScalar title_color);
...
void CPlot::title(std::string title_name,CvScalar title_color = cv::Scalar(0,0,0))
{
    int chw = 6;
//    int chh = 10;
...
void CPlot::xlabel(std::string xlabel_name, CvScalar label_color = cv::Scalar(0,0,0))
{
    int chw = 6;
    //    int chh = 10;
...
void CPlot::ylabel(std::string ylabel_name, CvScalar label_color = cv::Scalar(0,0,0))
{
    CvFont font;
    cvInitFont(&font,CV_FONT_HERSHEY_PLAIN,1.5,0.7, 0,1,CV_AA);
...
public:
    //重载这两个函数 传参简单
    template<class T>
    void plot(std::vector<T> Y,CvScalar color, char type = '*',bool is_need_lined = true);
    template<class T>
    void plot(std::vector< cv::Point_<T>  > p, CvScalar color,  char type = '*', bool is_need_lined = true);
    //增加一个函数把C版本的 IplImage 转换成Mat
    cv::Mat figure()
    {
        return cv::Mat(this->Figure);
    }
};
...
template<class T>
void Plot::plot(std::vector<T> Y, CvScalar color, char type,bool is_need_lined)
{
    //对数据进行存储
    T tempX, tempY;
	std::vector<CvPoint2D64f>data;
...
template<class T>
void Plot::plot(std::vector< cv::Point_<T> > p, CvScalar color, char type,bool is_need_lined)
{
    //对数据进行存储
    T tempX, tempY;
	std::vector<CvPoint2D64f>data;
