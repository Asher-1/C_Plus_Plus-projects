...
// ReadMesh
//********************************************
int CParserVrml::ReadMesh(TriMesh *Tmesh)
{
	// Check
	if(!CheckMesh())
		return 0;
  ...
  // diffuseColor 0.5686 0.1098 0.6941
	...
	vector<vec>  m_ArrayVertex;
	m_ArrayVertex.resize(NbVertex);
	vector<TriMesh::Face> m_ArrayFace;
	m_ArrayFace.resize(NbFace);
	...
	do
	{
		...
		success &= sscanf(m_pBufferWord,"%f",&z);
		if(success)
			m_ArrayVertex[NbVertex++]=vec(x,y,z);
		//TRACE("\n (%g %g %g) ",x,y,z);
	}
	while(success);
	...
	Tmesh->vertices=m_ArrayVertex;
	Tmesh->faces=m_ArrayFace;
...
// Run
//********************************************
int CParserVrml::Run(char *filename, TriMesh *Tmesh)
{
	TRACE("\n");
