...
void CDemo1View::Onlocation() 
{
	...
	charsRecognition charslocation(size,pDib->m_nBitCount,
			pDib->m_lpColorTable, pDib->m_pImgData);
	...
	CDemo1Doc* pDocNew=pView->GetDocument();
	ImageDib *dibNew=pDocNew->GetPDib();
	dibNew->ReplaceDib(size,charslocation.m_nBitCountOut,charslocation.m_lpColorTableOut, charslocation.m_pImgDataOut);
	pView->OnInitialUpdate();	
	pDocNew->SetModifiedFlag(TRUE);
	pDocNew->UpdateAllViews(pView);
}
