...
BOOL CMSMoneyDemoApp::InitInstance()
{
	...
	SetRegistryKey(_T("Microsoft\\MFC\\ImageProcessSystem0"));
	...
	m_pMainWnd->SetWindowText("数字图像处理系统");


	// 加载汉字识别库模板ChineseCrude.dat
	ifstream infile("ChineseCrude.dat", ios::binary);
	if (!infile)
	{
		AfxMessageBox("打开汉字文件失败");
	}
	infile.read((char*)(InitChineseCrude), sizeof(InitChineseCrude));
	infile.close();
