...
*   无
*
*说明：击中击不中变换,255(白)代表背景,0(黑)代表目标
***********************************************************************/
							...
							int imgWidth,int imgHeight, ElementPair hitMissMask)						 
{
	...
	//输出图像置255
	memset(imgBufOut, 255, lineByte*imgHeight);
		...
		for(i=0;i<8;i++){
			...
			for(k=0;k<m_imgHeight;k++){
				for(l=0;l<lineByte;l++){
					if(*(m_pImgDataOut+k*lineByte+l)==0){
