...
int charsRecognition::LineCluster()
{
    ...
    for(list <h> ::iterator it=m_ProjX.begin();it!=m_ProjX.end();++it){
		if(clustering.empty()){
			...
			vector <cluster> ::iterator itv;
			for( itv=clustering.begin();itv!=clustering.end();++itv){
...
void charsRecognition::MergeProjX(int lWm)
{
	...
	h *newX=new h;
	list<h>::iterator itx=m_ProjX.begin();
...
void charsRecognition::CutImage(int lWm)
{
...
void charsRecognition::GetMinRect(Position &pos)
{
	unsigned char*	lpSrc;				// 指向源图像的指针	
	LONG	i;							// 循环变量
	LONG	j;	
...
void charsRecognition::BlockNormalization()
{
	int i=0,j=0;
...
//汉字归一化 大小48
void charsRecognition::Norm(CFtChinese &itx)
{
	unsigned char*	lpSrc;				// 指向源图像的指针	
	LONG	i;							// 循环变量
	LONG	j;	
...
///////////////////////////////////////////////////////////////////////////
void charsRecognition::Normalization(Position &pos,int SWH,int *iImage)
{
...
////////////////////////////////////////////////////////////////////////
///*//一个像素宽
void charsRecognition::Thinning(int SWH,CFtChinese &it)
{
    int i=0,j=0;
    bool flag=true;
...
void charsRecognition::GetCrudeFeature()
{
	//Position pos;
	int i,j;
	double p1=0,p2=0;
...
//细分特征
//粗网格
void charsRecognition::CrossMesh(CFtChinese &it)
{
    LONG i,j,m,n;
	double sum=0;
	int count=0;
...
//粗外围
void charsRecognition::CrossPeriphery(CFtChinese &it)
{
...
void charsRecognition::CrudeMatchChinese()
{
...
//double InitChineseCrude[870][129];
...
void charsRecognition::DrawRect()
{
	unsigned char*	lpSrc;				// 指向源图像的指针	
	LONG	i;							// 循环变量
	LONG	j;	
